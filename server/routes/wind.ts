...
function ovUrl(variable: string, face: number): string {
...
function levelToAltitude(level: number): number {
  return (level / 52) * 80;
}

function temperatureAtLevel(level: number, lat: number): number {
  ...
  return (
    base -
    40 * Math.pow(Math.sin((lat * Math.PI) / 180), 2) +
    (Math.random() - 0.5) * 5
  );
}
...
function generateWindField(
  ...
  const jetFactor = level >= 20 && level <= 40 ? 2.5 : 1;
  const points = [];
...
router.get("/wind/vectors", async (req, res) => {
  ...
  res.json(
    generateWindField(face, level, resolution, Math.floor(resolution / 2), year)
  );
});
...
router.get("/wind/levels", async (req, res) => {
  ...
  for (let lev = 0; lev < 52; lev++) {
    ...
    const jetFactor =
      lev >= 20 && lev <= 40 ? 2.2 + rng() * 0.5 : 1;
    const avgU =
      10 * Math.cos((lev / 52) * Math.PI * 2) * jetFactor + (rng() - 0.5) * 5;
    const avgV =
      4 * Math.sin((lev / 52) * Math.PI) + (rng() - 0.5) * 3;
...
router.get("/wind/profile", async (req, res) => {
  ...
  for (let lev = 0; lev < 52; lev++) {
    const altKm = levelToAltitude(lev);
    const jetFactor = lev >= 22 && lev <= 38 ? 2.8 : 1;
    const u =
...
router.get("/wind/bar", async (req, res) => {
  ...
  for (const lev of selectedLevels) {
    for (const region of regions) {
      const altKm = levelToAltitude(lev);
      const jetFactor = lev >= 20 && lev <= 40 ? 2 + rng() * 0.8 : 1;
...
router.get("/wind/particles", async (req, res) => {
  ...
  if (uv) {
    ...
    const allU = uField.filter(isFinite),
      allV = vField.filter(isFinite);
    return res.json({
      width,
      height,
      uField,
